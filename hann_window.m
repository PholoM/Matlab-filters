...
lp_filtered = conv(s1, hwhn);

audiowrite("Hanning LP.wav", lp_filtered, 44100);
...
hpd = [hpb 1-2*Fc hpf];
hpwhn = hpd.*whn;
hp_filtered = conv(s1, hpwhn);
audiowrite("Hanning HP.wav", hp_filtered, 44100);
...
bpwhn = bpd.*whn;
bp_filtered = conv(s1, bpwhn);
audiowrite("Hanning BP.wav", bp_filtered, 44100);
...
bswhn = bsd.*whn;
bs_filtered = conv(s1, bswhn);
audiowrite("Hanning BS.wav", bs_filtered, 44100);
